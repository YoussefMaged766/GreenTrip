...
@HiltViewModel
class ResetPasswordViewModel @Inject constructor(private val userRepo: AuthRepo) : ViewModel() {

    private val _state = MutableStateFlow(AuthState())
    val state = _state.asSharedFlow()

    fun resetPassword(token: String, user: AuthModel) =
        viewModelScope.launch {
            userRepo.resetPassword(token, user).collect {
                when (it) {
                    is Status.Loading -> {
