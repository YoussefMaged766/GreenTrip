...
import com.example.greentrip.ui.main.profile.ProfileViewModel
import java.io.*
import java.util.*


internal fun getFilePathFromUri(context: Context, uri: Uri,profileViewModel: ProfileViewModel): String? =
    if (uri.path?.contains("file://") == true) {
        uri.path
    } else {
        getFileFromContentUri(context, uri,profileViewModel).path

    }

@SuppressLint("SuspiciousIndentation")
private fun getFileFromContentUri(context: Context, contentUri: Uri, profileViewModel: ProfileViewModel): File {
    ...
    profileViewModel.setFileName(fileName)
